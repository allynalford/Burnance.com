...
const getPreviousMap = async (previousMapDownloadUrl) => {
  try {
    const map = await got.get(previousMapDownloadUrl, null).json();
    return map;
  } catch (e) {
    return undefined;
  }
  ...
  try {
    ...
    await new Promise(async (resolve, reject) => {
      ...
      if (filesPaths.length === 0) {
        reject(
          new Error('当前应用配置文件 serverless.yml 中指定的 src 目录无效或为空，请检查后重试')
        );
      }
...
const cache = async (sdk, instance, options) => {
  ...
  if (filesToUpload.length !== 0 || filesToDelete.length !== 0) {
    ...
    try {
      await got.put(changesUploadUrl, {
        body: zipBuffer,
      });
    } catch (e) {
      e.extraErrorInfo = {
...
const run = async (sdk, method = {}, instanceData = {}, credentials = {}, options = {}) => {
  ...
  if (instance.inputs.src && typeof instance.inputs.src === 'string' && method === 'deploy') {
    try {
      const { srcDownloadUrl } = await cache(sdk, instance, options);
