...
class Collection extends Component {
  ...
  componentDidMount() {
    ...
    if (window.ethereum) {
      ...
      if (window.ethereum._state.isConnected && typeof window.ethereum._state.accounts[0] !== "undefined") {
        ...
        this.getNFTs(window.ethereum._state.accounts[0], this.props.match.params.address);
      }
    } 
  }
  ...
  getNFTs = async (ethereumAddress, contractAddress) => {
    ...
    try {
      ...
      const exists = JSON.parse(sessionstorage.getItem(ethereumAddress+'-'+contractAddress));

      console.log(exists)

      //Check if the records exists in storage
      if(typeof exists !== "undefined" && exists !== null){
        this.setState({collection: exists.collection, nfts: exists.nfts, loading: false})
      }else{
      //Call the service to get the NFTs
      const Collection = await endpoint._get(getChain()['eth'].viewWalletCollectionApiUrl + `/ethereum/${ethereumAddress}/${contractAddress}`);
      this.setState({collection: Collection.data.collection, nfts: Collection.data.nfts, loading: false})
      
      sessionstorage.setItem(ethereumAddress+'-'+contractAddress, JSON.stringify(Collection.data));
      }
    } catch (e) {
      console.error(e);
    }
